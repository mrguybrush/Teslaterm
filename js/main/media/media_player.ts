...
type SongEndedListener = () => void;
const songEndedListeners: SongEndedListener[] = [];

export function onSongEnded(cb: SongEndedListener) {
    songEndedListeners.push(cb);
}

export function notifySongEnded() {
    songEndedListeners.forEach((cb) => cb());
}
