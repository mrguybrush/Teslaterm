...
import {disconnectFrom, getConnectionState, getCoilCommands} from "../connection/connection";
...
export class PerCoilMenuIPC {
    ...
    constructor(processIPC: TemporaryIPC, coil: CoilID) {
        ...
        processIPC.on(mainIPCs.menu.requestSingleConfig, (paramName) => {
            requestConfig(this.coil, (cfg) => {
                const match = cfg.find((c) => c.name === paramName);
                if (match) {
                    this.processIPC.send(this.renderIPCs.singleConfigValue, match);
                }
            });
        });
        processIPC.on(mainIPCs.menu.requestConfigList, () => {
            requestConfig(this.coil, (cfg) => this.processIPC.send(this.renderIPCs.configList, cfg));
        });
        processIPC.on(mainIPCs.menu.disconnect, async (source) => {
            try {
                await getCoilCommands(coil).setKill();
                await new Promise((resolve) => setTimeout(resolve, 200));
            } catch (err) {
                console.error("While killing coil before disconnect", err);
            }
            await disconnectFrom(coil);
        });
