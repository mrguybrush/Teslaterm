...
let configRequestTimeout: ReturnType<typeof setTimeout> | undefined;
...
export function requestConfig(coil: CoilID, out: UD3ConfigConsumer) {
    ...
    if (configRequestQueue.length === 1) {
        ...
        clearTimeout(configRequestTimeout);
        configRequestTimeout = setTimeout(() => {
            console.warn("Timed out waiting for config_get response from coil, clearing pending config requests");
            const pending = configRequestQueue;
            configRequestQueue = [];
            udconfig = [];
