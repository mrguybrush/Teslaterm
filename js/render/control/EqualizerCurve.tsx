...
export interface EqualizerCurveProps {
    points: EqPoint[];
    enabled: boolean;
    // Lifted to the parent (rather than kept as local state, as it used to be) so a sidebar next to
    // the curve - e.g. a Q slider for people without a mouse wheel - can act on the same point.
    selectedId?: number;
    onSelectPoint: (id: number | undefined) => void;
...
// The golden angle spreads hues about as far apart as possible for however many points exist,
// rather than cycling a short fixed palette that repeats (and collides) once there are more points
// than colours. Id-keyed, not index-keyed, so a point keeps its colour as others are added/removed.
const GOLDEN_ANGLE_DEG = 137.50776;

function pointColor(id: number, enabled: boolean): string {
    if (!enabled) {
        return '#8a8a8a';
    }
    const hue = (id * GOLDEN_ANGLE_DEG) % 360;
...
export class EqualizerCurve extends TTComponent<EqualizerCurveProps, EqualizerCurveState> {
    ...
    private onMouseDown(ev: React.MouseEvent) {
        ...
        this.props.onSelectPoint(hit?.id);
        if (hit) {
    ...
    private readonly onWindowKeyDown = (ev: KeyboardEvent) => {
        if ((ev.key === 'Delete' || ev.key === 'Backspace') && this.props.selectedId !== undefined) {
            // Only when the focus isn't in some other input (the sidebar's Q slider included) - so
            // this never eats a Backspace meant for actual text/slider editing elsewhere.
            const target = document.activeElement;
            if (target && (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA')) {
                return;
            }
            this.removePoint(this.props.selectedId);
        }
    };
    ...
    private onDoubleClick(ev: React.MouseEvent) {
        ...
        this.props.onSelectPoint(point.id);
    ...
    private removePoint(id: number) {
        ...
        if (this.props.selectedId === id) {
            this.props.onSelectPoint(undefined);
        }
    }
    ...
    private drawGrid(ctx: CanvasRenderingContext2D, width: number, height: number) {
        ...
        for (const freq of GRID_FREQUENCIES) {
            ...
            ctx.stroke();
            ctx.fillStyle = '#888';
            // A row of its own well above the very bottom edge - the 0% gain gridline's label sits
            // right at that edge (see below), and at the low end of the frequency axis the two used
            // to land almost exactly on top of each other, painting over each other into unreadable
            // overlapping text.
            ctx.fillText(`${formatFreq(freq)} Hz`, x + 3, height - 16);
        }
    ...
    private drawPoints(ctx: CanvasRenderingContext2D, width: number, height: number) {
        for (const point of this.props.points) {
            ...
            const active = point.id === this.props.selectedId || point.id === this.state.hoveredId
                || point.id === this.draggingId;

            ctx.beginPath();
            ctx.arc(x, y, active ? POINT_RADIUS + 2 : POINT_RADIUS, 0, Math.PI * 2);
            ctx.fillStyle = pointColor(point.id, this.props.enabled);
            ctx.fill();
            ctx.lineWidth = 2;
